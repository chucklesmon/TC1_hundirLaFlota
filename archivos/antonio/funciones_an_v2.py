    ...
    for indice in range(longitud):
        if curso == 1:
            if coord_f + longitud <= 9:
                if tablero_juego[coord_f + indice, coord_c] == "\u23F9":
                    posible_form = False
        elif curso == 2:
            if coord_c + longitud <= 9:
                if tablero_juego[coord_f, coord_c + indice] == "\u23F9":
                    posible_form = False
...
def comprobar_dist_barcos(longitud, coord_f, coord_c, curso, tablero_juego):
    ...
    while indice < longitud + 1:
        if curso == 1 and coord_f + indice <= 9:
            if tablero_juego[coord_f + indice, coord_c - 1] == "\u23F9" or tablero_juego[coord_f + indice, coord_c + 1] == "\u23F9":
                posible_esp = False
        elif curso == 2 and coord_c + indice <= 9:
            if tablero_juego[coord_f - 1, coord_c + indice] == "\u23F9" or tablero_juego[coord_f + 1, coord_c + indice] == "\u23F9":
                posible_esp = False 
        indice += 1
    ...
    if curso == 1:
        if coord_f != 0 and coord_f + longitud <= 9:
            if tablero_juego[coord_f - 1, coord_c] == "\u23F9" or tablero_juego[coord_f + longitud, coord_c] == "\u23F9":
                posible_esp = False
        elif coord_f == 0:
            if tablero_juego[coord_f + longitud, coord_c] == "\u23F9":
                posible_esp = False
        elif coord_f + longitud > 9:
            posible_esp = False
    elif curso ==2:
        if coord_c != 0 and coord_c + longitud <= 9:
            if tablero_juego[coord_f, coord_c - 1] == "\u23F9" or tablero_juego[coord_f, coord_c + longitud] == "\u23F9":
                posible_esp = False
        elif coord_c == 0:
            if tablero_juego[coord_f, coord_c + longitud] == "\u23F9":
                posible_esp = False
...
def situar_en_tablero(longitud, coord_f, coord_c, curso, tablero_juego):
    ...
    for indice in range(longitud):
        if curso == 1:
            tablero_juego[coord_f + indice, coord_c] = "\u23F9"
        elif curso == 2:
            tablero_juego[coord_f, coord_c + indice] = "\u23F9" #"\u220E" Pata tachar
